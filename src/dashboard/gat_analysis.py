"""
GAT-Specific Analysis Dashboard for Graph Attention Network interpretation.
"""
...
def create_gat_dashboard(model, dataset, full_data, device='cpu', atomwise_dft_dim=6):
    """
    ...
    model : torch.nn.Module
        Trained GAT model
    dataset : list
        List of PyG graph data objects. Must have .name and .smiles attributes.
    full_data : dict
        Dictionary mapping molecule names to DFT data properties
    device : str or torch.device
        Device to run inference on
    atomwise_dft_dim : int
        Dimension of DFT features used in node representation (default: 6)
    """
    ...
    # Ensure get_atom_importance is available or re-implement simple version
    # Since original code called get_atom_importance(model, graph), we assume it exists globally 
    # or should be implemented here. For robustness, I'll implement a fallback if not passed,
    # but the original snippet implies it was available. I'll define a local version using attention.
    
    def get_atom_importance(model, graph):
        """Estimate atom importance based on aggregated attention weights."""
        try:
            attn_data = get_attention_weights(model, graph)
            # Use Layer 2 attention as proxy for importance (aggregation from neighbors)
            # Summing incoming attention for each node
            attention = attn_data['layer2']['attention']
            edge_index = attn_data['layer2']['edge_index']
            ...
            for i in range(edge_index.shape[1]):
                ...
                if dst < n_atoms:
                    if len(attention.shape) > 1:
                        importance[dst] += attention[i].mean()
                    else:
    ...
    def get_node_features_analysis(graph):
        ...
        # Feature dimensions
        rdkit_dim = 33  # RDKit features - hardcoded in original snippet, kept for consistency
    ...
    def get_edge_features_analysis(graph, mol_name):
        ...
        # Assuming last feature is Mayer BO as per original code context
    ...
    # Molecule selector
    # Limit to top 50 or full dataset if smaller
    limit = min(50, len(dataset))
    mol_options = []
    
    # Fallback if name attribute is missing or different structure
    for i in range(limit):
        g = dataset[i]
        name = getattr(g, 'name', str(i))
        mol_options.append((f"#{i+1}: {name}", name))
    ...
    mol_graph_lookup = {getattr(g, 'name', str(i)): g for i, g in enumerate(dataset)}
    ...
    def update_attention_flow(change=None):
        with attention_flow_output:
            ...
            if layer != 'both':
                axes = [axes]
            elif not isinstance(axes, np.ndarray): # Handle single subplot edgecase
                axes = [axes]
            
            layers_to_plot = ['layer1', 'layer2'] if layer == 'both' else [layer]
            
            # Helper to safely iterate axes
            ax_iter = axes.flatten() if isinstance(axes, np.ndarray) else axes

            for ax, lyr in zip(ax_iter, layers_to_plot):
                ...
                if weights:
                    # Draw edges
                    max_w = max(weights) + 1e-6
                    edge_colors = [plt.cm.Reds(w/max_w) for w in weights]
            ...
            sm.set_array([])
            try:
                plt.colorbar(sm, ax=ax_iter[-1], label='Attention Weight', shrink=0.8)
            except: pass
    ...
    def update_edge_heatmap(change=None):
        with edge_heatmap_output:
            ...
            mol = Chem.MolFromSmiles(getattr(graph, 'smiles', ''))
            if not mol: return
            ...
            try:
                ...
                display(HTML(f"<div style='color:orange;'>Error analyzing edges (ensure bond features are present): {str(e)}</div>"))
    ...
    def update_feature_attr(change=None):
        with feature_attr_output:
            ...
            try:
                ...
                display(HTML(f"<div style='color:orange'>Error extracting features: {e}</div>"))
                return
            ...
            if feat_data['rdkit_mean'].size > 0 and feat_data['dft_mean'].size > 0:
            ...
            if dft_features.size > 0:
                ...
                if len(importance) == 0: importância = np.zeros(n_atoms)
                
                correlations = []
                for i in range(dft_features.shape[1]):
                    if len(importance) == len(dft_features[:, i]):
                        # Avoid NaNs in correlation
                        std_imp = np.std(importance)
                        std_feat = np.std(dft_features[:, i])
                        if std_imp > 0 and std_feat > 0:
    ...
    def update_dft_analysis(change=None):
        with dft_analysis_output:
            ...
            # Handle name cleaning automatically
            clean_name = mol_name.replace('_optimized', '')
            mol_key = None
            if mol_name in full_data: mol_key = mol_name
            elif mol_name + '_optimized' in full_data: mol_key = mol_name + '_optimized'
            elif clean_name in full_data: mol_key = clean_name
            
            if not mol_key:
                display(HTML(f"<div style='color:orange;'>⚠️ DFT data not found for {mol_name} in 'full_data'</div>"))
                return
            
            # Robustly access data based on varying structures
            data_entry = full_data[mol_key]
            atom_props = data_entry.get('atom_properties', {}) if isinstance(data_entry, dict) else {}
            feature_values = atom_props.get(feature_name, [])[:n_atoms]
            
            if len(feature_values) == 0:
                display(HTML(f"<div style='color:orange;'>⚠️ Feature {feature_name} not available or empty</div>"))
                return
    ...
    def update_network(btn=None):
        with network_output:
            ...
            try:
                ...
                display(HTML(f"<div style='color:orange;'>Error generating network view: {e}</div>"))
    ...
    header = widgets.HTML("""
        <div style='background:linear-gradient(135deg,#1a237e,#311b92);padding:20px;border-radius:12px;color:white;margin-bottom:15px;'>
            <h1 style='margin:0;' </h1>GAT-Specific Analysis Dashboard</h1>
    ...
    # Initialize
    # To avoid heavy computation on load, we can leave tabs to update on demand or trigger the first one
    update_attention_flow()
